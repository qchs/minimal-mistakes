...
| 知识点                                                       | 疑问                                                         |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| 异常的捕获                                                   | 异常分哪些？Error(路塌了)，Exception(刹车坏了，必须处理,五种常见-除以0，数组下标越界，文件不存在，try-catch-finally，throws-不知道如何处理，交给上级，不太好的编程习惯)，RuntimeException(轮胎里有小石子) |
| 文件reader和writer都是使用了系统资源，这种资源java不能自动回收，所以需要明确的指令来告诉系统，这个东西我不用了，给其他人用吧。 关闭方法：调用各自的close方法即可。 | 资源的清除，关闭，删临时文件                                 |
...
public class Main3 {
    ...
    static void print(ArrayList al) {
        int length = al.size();
        //前三个是，。和回车
        //可以增加一个判断，如果是标点符号，不打印，或者直接不参与计数
        for (int i = 0; i < length; i++) {
            if (i %7 == 0) {
                System.out.println();
            }
...
| 运行时错误                                                   | 解决方法                                                     |
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| 当列表中还没有一个数据时，长度为0，对其进行长度处理时会发生越界 | 当长度为零时，直接把第一个字母加进来，第一个程序为什么没有这个报错？对起始和结束条件要一样多想一步 |
