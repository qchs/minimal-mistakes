...
| 接口 Map.Entry<K,V>,hashMap.entrySet()的返回set的类型限制    |                                                              |
...
| git commit -m与git commit -a -m 的区别                       | 有了git add .  下一句就不需要-a                              |
| Map.Entry<Charactor,Integer> m : list                        |                                                              |
| hashMap的`**entrySet**()方法` 返回此映射所包含的映射关系的 [`Set`](http://tool.oschina.net/uploads/apidocs/jdk-zh/java/util/Set.html) 视图。类中还有setValue()方法，可以改变值，无法改变键 | ![1530776407694](../assets/images/1530776407694.png)一个不包含重复元素的 collection。更确切地讲，set 不包含满足 `e1.equals(e2)` 的元素对 `e1` 和 `e2`，并且最多包含一个 null 元素。正如其名称所暗示的，此接口模仿了数学上的 *set* 抽象。 |
...
| list里面可以放set？                                          | `**ArrayList**(Collection<? extends E> c)`             构造一个包含指定 collection 的元素的列表，这些元素是按照该 collection 的迭代器返回它们的顺序排列的。 |
